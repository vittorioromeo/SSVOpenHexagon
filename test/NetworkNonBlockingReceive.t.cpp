...
int main()
{
    ...
    //   [u32 payload size, network byte order][payload bytes]
    // We send these bytes ourselves so we control exactly where the split
    // happens. 8-byte payload → size prefix is {0x00, 0x00, 0x00, 0x08}.
    const unsigned char   payload[8]    = {0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE};
    const sf::base::SizeT payloadSize   = sizeof(payload);
    const unsigned char   sizePrefix[4] = {0x00, 0x00, 0x00, static_cast<unsigned char>(payloadSize)};
