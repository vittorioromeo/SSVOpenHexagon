...
void HexagonGame::initLua()
{
    ...
    lua.writeVariable("m_messageAddImportant", [=](string mMsg, float mDuration)
    {
        eventTimeline.append<Do>([=]
        {
                      ...
                      float mMinSpeed, float mMaxSpeed)
    {
        timeline.append<Do>([=]
        {
            ...
            mMinSpeed * getSpeedMultDM(), mMaxSpeed * getSpeedMultDM()});
        });
    });
                      ...
                      float mSMin, float mSMax, bool mSPingPong)
    {
        ...
        {
            factory.createWall(mSide, mThickness,
            {mSAdj * getSpeedMultDM(), mSAcc, mSMin, mSMax, mSPingPong}, mHMod);
        });
    });
