...
{
    for(const auto& t :
    {"titleBar.png", "creditsBar1.png", "creditsBar2.png", "creditsBar2b.png",
    "creditsBar2c.png", "creditsBar2d.png", "bottomBar.png"})
...
void MenuGame::initMenus()
{
    ...
    whenMusicEnabled;
    sfx.create<i::Slider>("music speed multipler", &Config::getMusicSpeedMult,
    [this](float mValue)
    {
...
void MenuGame::initLua(Lua::LuaContext& mLua)
{
    ...
    for(const auto& un : {"l_setSpeedMult", "l_setSpeedInc",
        ...
        "l_setPulseMin", "l_setPulseMax", "l_setPulseSpeed", "l_setPulseSpeedR",
        ...
        "w_wallAdj", "w_wallAcc", "w_wallHModSpeedData", "w_wallHModCurveData",
        "l_setDelayMult", "l_setMaxInc"})
...
void MenuGame::draw()
{
    ...
    if(state == s::SMain)
        styleData.drawBackground(window, ssvs::zeroVec2f, levelStatus.sides);
...
void MenuGame::drawLevelSelection()
{
    ...
    if(Config::getOnline()) {
        ...
        Text& lbest =
        renderText(lbestStr, txtProf, {20.f, getGlobalBottom(pack) - 7.f}, 18);
        if(diffMults.size() > 1)
            renderText(
            "difficulty: " + toStr(ssvu::getByModIdx(diffMults, diffMultIdx)),
...
{
    renderText(
    mMenu.getCategory().getName(), txtLDesc, {20.f, getGlobalBottom(titleBar)});
