...
{
    const auto rndReal = [&rng]() -> float { return rng.get_real<float>(0, 1); };

    const auto rndIntUpper = [&rng](int upper) -> float { return rng.get_int<int>(1, upper); };

    const auto rndInt = [&rng](int lower, int upper) -> float { return rng.get_int<int>(lower, upper); };

    addLuaFn(lua, "u_rndReal", rndReal).doc("Return a random real number in the [0; 1] range.");
...
{
    hg::lo("hg::LuaScripting::redefineIoOpen") << "Failure to redefine Lua's `io.open` function\n";
...
{
    hg::lo("hg::LuaScripting::redefineRandom") << "Failure to redefine Lua's `math.random` function\n";
...
static void initUtils(Lua::LuaContext& lua, const bool inMenu, const bool headless)
{
    addLuaFn(lua,
             "u_inMenu", //
...
static void initCustomWalls(Lua::LuaContext& lua, CCustomWallManager& cwManager)
{
    ...
    addLuaFn(lua,
             "cw_createDeadly", //
             [&cwManager]() -> CCustomWallHandle
    { return cwManager.create([](CCustomWall& cw) { cw.setDeadly(true); }); })
        .doc(
    ...
    addLuaFn(lua,
             ...
             [&cwManager]() -> CCustomWallHandle
    { return cwManager.create([](CCustomWall& cw) { cw.setCanCollide(false); }); })
        .doc(
    ...
    addLuaFn(lua,
             "cw_destroy", //
             [&cwManager](CCustomWallHandle cwHandle) { cwManager.destroy(cwHandle); })
        .arg("cwHandle")
        .doc("Destroy the custom wall represented by `$0`.");

    addLuaFn(lua,
             "cw_setVertexPos", //
             [&cwManager](CCustomWallHandle cwHandle, int vertexIndex, float x, float y)
    ...
    addLuaFn(lua,
             "cw_setVertexColor", //
             [&cwManager](CCustomWallHandle cwHandle, int vertexIndex, int r, int g, int b, int a)
    { cwManager.setVertexColor(cwHandle, vertexIndex, sf::Color(r, g, b, a)); })
        .arg("cwHandle")
        .arg("vertexIndex")
        .arg("r")
        .arg("g")
    ...
    addLuaFn(lua,
             "cw_setCollision", //
             [&cwManager](CCustomWallHandle cwHandle, bool collision) { cwManager.setCanCollide(cwHandle, collision); })
        .arg("cwHandle")
        .arg("collision")
        .doc(
    ...
    addLuaFn(lua,
             "cw_setDeadly", //
             [&cwManager](CCustomWallHandle cwHandle, bool deadly) { cwManager.setDeadly(cwHandle, deadly); })
        .arg("cwHandle")
        .arg("deadly")
        .doc(
    ...
    addLuaFn(lua,
             "cw_setKillingSide", //
             [&cwManager](CCustomWallHandle cwHandle, unsigned int side) { cwManager.setKillingSide(cwHandle, side); })
        .arg("cwHandle")
        .arg("side")
        .doc(
    ...
    addLuaFn(lua,
             "cw_getCollision", //
             [&cwManager](CCustomWallHandle cwHandle) -> bool { return cwManager.getCanCollide(cwHandle); })
        .arg("cwHandle")
        .doc(
    ...
    addLuaFn(lua,
             "cw_getDeadly", //
             [&cwManager](CCustomWallHandle cwHandle) -> bool { return cwManager.getDeadly(cwHandle); })
        .arg("cwHandle")
        .doc(
    ...
    addLuaFn(lua,
             "cw_getKillingSide", //
             [&cwManager](CCustomWallHandle cwHandle) -> unsigned int { return cwManager.getKillingSide(cwHandle); })
        .arg("cwHandle")
        .doc(
    ...
    addLuaFn(lua,
             "cw_getVertexPos", //
             [&cwManager](CCustomWallHandle cwHandle, int vertexIndex) -> std::tuple<float, float>
    {
    ...
    addLuaFn(lua,
             "cw_getVertexPos4", //
             [&cwManager](CCustomWallHandle cwHandle) -> std::tuple<float, float, float, float, float, float, float, float>
    {
...
static void initLevelControl(Lua::LuaContext& lua, LevelStatus& levelStatus, HexagonGameStatus& status)
{
    const auto lsVar = makeLuaAccessor(lua, levelStatus, "l");

    lsVar("SpeedMult",
          &LevelStatus::speedMult,
    ...
    lsVar("RotationSpeedInc",
          &LevelStatus::rotationSpeedInc,
    ...
    lsVar("RotationSpeedMax",
          &LevelStatus::rotationSpeedMax,
    ...
    lsVar("PulseInitialDelay",
          &LevelStatus::pulseInitialDelay,
    ...
    lsVar("SwapCooldownMult",
          &LevelStatus::swapCooldownMult,
    ...
    lsVar("BeatPulseDelayMax",
          &LevelStatus::beatPulseDelayMax,
    ...
    lsVar("BeatPulseInitialDelay",
          &LevelStatus::beatPulseInitialDelay,
    ...
    lsVar("WallAngleRight",
          &LevelStatus::wallAngleRight,
    ...
    lsVar("ShadersRequired",
          &LevelStatus::shadersRequired,
    ...
    lsVar("ManualPulseControl",
          &LevelStatus::manualPulseControl,
    ...
    lsVar("ManualBeatPulseControl",
          &LevelStatus::manualBeatPulseControl,
    ...
    addLuaFn(lua,
             "l_enableRndSideChanges", //
             [&levelStatus](bool mValue) { levelStatus.rndSideChangesEnabled = mValue; })
        .arg("enabled")
        .doc(
    ...
    addLuaFn(lua,
             ...
             [&levelStatus]
    {
        levelStatus.trackedVariables.clear();
    ...
    sVar("PulseDirection",
         &HexagonGameStatus::pulseDirection,
    ...
    sVar("BeatPulseDelay",
         &HexagonGameStatus::beatPulseDelay,
    ...
    sVar("ShowPlayerTrail",
         &HexagonGameStatus::showPlayerTrail,
...
static void initStyleControl(Lua::LuaContext& lua, StyleData& styleData)
{
    ...
    sdVar("3dPerspectiveMult",
          &StyleData::_3dPerspectiveMult,
    ...
    addLuaFn(lua,
             "s_setCapColorMainDarkened", //
             [&styleData] { styleData.setCapColor(CapColor{CapColorMode::MainDarkened{}}); })
        .doc(
            "Set the color of the center polygon to match the main style "
            "color, darkened.");
    ...
    addLuaFn(lua,
             ...
             [&styleData](int mIndex) { styleData.setCapColor(CapColor{CapColorMode::ByIndex{mIndex}}); })
        .arg("index")
        .doc(
    ...
    const auto sdColorGetter = [&lua, &styleData, &colorToTuple](const char* name, const char* docName, auto pmf)
    {
        ...
        }).doc(Utils::concat("Return the current ", docName, " color computed by the level style."));
    };
    ...
    sdColorGetter("s_get3DOverrideColor", "3D override", &StyleData::get3DOverrideColor);

    sdColorGetter("s_getCapColorResult", "cap color result", &StyleData::getCapColorResult);
                        ...
                        const bool                                      headless)
{
                 ...
                 const sf::base::String& shaderFilename) -> sf::base::SizeT
    {
        ...
        if (!id.hasValue())
        {
            hg::lo("hg::LuaScripting::initShaders")
                << "`u_getShaderId` failed, no id found for '" << shaderFilename << "'\n";
                        ...
                        const sf::base::String& shaderFilename) -> sf::base::SizeT
    {
        ...
        auto setResult = [&assets, &result](const sf::base::String& shaderPath)
        {
            ...
            {
                hg::lo("hg::LuaScripting::initShaders")
                    << "`u_getDependencyShaderId` failed, no id found for '" << shaderPath << "'\n";
    ...
    auto withValidShaderId = [&assets, headless](const char* caller, const sf::base::SizeT shaderId, auto&& f)
    {
        if (headless)
        {
            // Always return early in headless mode.
    ...
    const auto checkValidRenderStage = [headless](const char* caller, const sf::base::SizeT renderStage, auto& ids) -> bool
    {
        ...
        {
            hg::lo("hg::LuaScripting::initShaders")
                << "`" << caller << "` failed, invalid render stage id '" << renderStage << "'\n";
             ...
             [&hexagonGameStatus]()
    {
        ...
        for (sf::base::SizeT i = 0; i < static_cast<sf::base::SizeT>(RenderStage::Count); ++i)
        {
    ...
    addLuaFn(lua,
             "shdr_resetActiveFragmentShader",
             [checkValidRenderStage, &hexagonGameStatus](const sf::base::SizeT renderStage)
    {
        auto& ids = hexagonGameStatus.fragmentShaderIds;

        if (checkValidRenderStage("shdr_resetActiveFragmentShader", renderStage, ids))
        {
    ...
    addLuaFn(lua,
             "shdr_setActiveFragmentShader",
             [checkValidRenderStage, &hexagonGameStatus](const sf::base::SizeT renderStage, const sf::base::SizeT shaderId)
    {
        auto& ids = hexagonGameStatus.fragmentShaderIds;

        if (checkValidRenderStage("shdr_setActiveFragmentShader", renderStage, ids))
        {
          ...
          const bool                                          headless)
{
    ...
    initExecScript(lua, assets, fRunLuaFile, execScriptPackPathContext, fPackPathGetter, fGetPackData);
    initShaders(lua, assets, execScriptPackPathContext, fPackPathGetter, fGetPackData, hexagonGameStatus, headless);
    initConfig(lua);
}
