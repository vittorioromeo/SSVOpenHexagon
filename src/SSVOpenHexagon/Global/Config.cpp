...
namespace hg
{
    namespace Config
    {
        ...
        auto& windowedAutoResolution(
            lvm.create<bool>("windowed_auto_resolution"));
        auto& fullscreenAutoResolution(
            lvm.create<bool>("fullscreen_auto_resolution"));
        ...
        void loadConfig(const vector<string>& mOverridesIds)
        {
            ...
            for(const auto& p :
                getScan<ssvufs::Mode::Single, ssvufs::Type::File,
                    ssvufs::Pick::ByExt>("ConfigOverrides/", ".json"))
            {
                if(contains(mOverridesIds, p.getFileNameNoExtensions()))
                {
                    ...
                    for(auto itr(begin(overrideRoot)); itr != end(overrideRoot);
                        ++itr)
                        root[getKey(itr)] = *itr;
                }
            }
