...
void StyleData::drawBackground(
RenderTarget& mRenderTarget, const Vec2f& mCenterPos, unsigned int mSides)
{
    ...
    for(auto i(0u); i < mSides; ++i) {
        ...
        getOrbitRad(mCenterPos, angle + div * 0.5f, distance), currentColor);
        vertices.emplace_back(
        getOrbitRad(mCenterPos, angle - div * 0.5f, distance), currentColor);
    }
