...
void drawOnlineScreen(Context& ctx, App& app, Services& svc)
{
    ...
    Item items[6];
    int  count = 0;
    if (snap.canConnect)    items[count++] = {"CONNECT",    &svc.onOnlineConnect};
    if (snap.canLogIn)      items[count++] = {"LOGIN",      &svc.onOnlineLogin};
    if (snap.canRegister)   items[count++] = {"REGISTER",   &svc.onOnlineRegister};
    if (snap.canLogOut)     items[count++] = {"LOGOUT",     &svc.onOnlineLogout};
    if (snap.canDisconnect) items[count++] = {"DISCONNECT", &svc.onOnlineDisconnect};
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            s.selectedIdx = i;
            if (items[i].cb == nullptr) { goBack(app); return; }
            if (*items[i].cb) (*items[i].cb)();
        }
    }
}
