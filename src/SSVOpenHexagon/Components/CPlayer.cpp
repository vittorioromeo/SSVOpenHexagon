...
CPlayer::CPlayer(Entity& mE, HexagonGame& mHexagonGame, const Vec2f& mStartPos)
...
void CPlayer::draw()
{
    ...
    if(!swapTimer.isRunning())
        colorMain =
        ssvs::getColorFromHSV((swapBlinkTimer.getCurrent() * 15) / 360.f, 1, 1);
...
void CPlayer::update(FT mFT)
{
    ...
    if(deadEffectTimer.update(mFT) && hexagonGame.getLevelStatus().tutorialMode)
    ...
    if(hexagonGame.getLevelStatus().swapEnabled && hexagonGame.getInputSwap() &&
       !swapTimer.isRunning())
    {
