...
namespace ssvuj
{
	...
	namespace Impl
	{
		...
		{
			extrArrayHelper<TIdx>(mArray, mArg); extrArrayHelper<TIdx + 1>(mArray, FWD(mArgs)...);
		}
		...
		{
			archArrayHelper<TIdx>(mArray, mArg); archArrayHelper<TIdx + 1>(mArray, FWD(mArgs)...);
		}
		...
		{
			extrObjHelper(mObj, mKey, mArg); extrObjHelper(mObj, FWD(mArgs)...);
		}
		...
		{
			archObjHelper(mObj, mKey, mArg); archObjHelper(mObj, FWD(mArgs)...);
		}
	}
	...
	template<typename... TArgs> inline void extrArray(const Obj& mArray, TArgs&... mArgs) { Impl::extrArrayHelper<0>(mArray, FWD(mArgs)...); }
	...
	template<typename... TArgs> inline void archArray(Obj& mArray, const TArgs&... mArgs) { Impl::archArrayHelper<0>(mArray, FWD(mArgs)...); }

	/// @brief Returns a new JSON array instance with any number of values archived in it.
	/// @param mArgs Const references to the values to archive.
	template<typename... TArgs> inline Obj getArchArray(const TArgs&... mArgs) { Obj result; archArray(result, FWD(mArgs)...); return result; }
	...
	template<typename... TArgs> inline void extrObj(const Obj& mObj, TArgs&... mArgs) { Impl::extrObjHelper(mObj, FWD(mArgs)...); }
	...
	template<typename... TArgs> inline void archObj(Obj& mObj, const TArgs&... mArgs) { Impl::archObjHelper(mObj, FWD(mArgs)...); }

	/// @brief Returns a new JSON Obj instance with any number of values archived in it.
	/// @param mArgs For every value archived, pass a key and a const reference to the value.
	template<typename... TArgs> inline Obj getArchObj(const TArgs&... mArgs) { Obj result; archObj(result, FWD(mArgs)...); return result; }
	...
	template<typename... TArgs> inline void convertArray(const Obj& mObj, TArgs&... mArgs) { extrArray(mObj, FWD(mArgs)...); }

	/// @brief Archives/extracts any number of values into/from a JSON array, depending from the constness.
	/// @param mObj Object to archive.
	/// @param mArgs Values to archive.
	template<typename... TArgs> inline void convertArray(Obj& mObj, const TArgs&... mArgs) { archArray(mObj, FWD(mArgs)...); }

	/// @brief Archives/extracts any number of values into/from a JSON Obj, depending from the constness.
	/// @param mObj Object to extract.
	/// @param mArgs For every value extracted, pass a key and a reference to the value.
	template<typename... TArgs> inline void convertObj(const Obj& mObj, TArgs&... mArgs) { extrObj(mObj, FWD(mArgs)...); }

	/// @brief Archives/extracts any number of values into/from a JSON Obj, depending from the constness.
	/// @param mObj Object to archive.
	/// @param mArgs For every value archived, pass a key and a const reference to the value.
	template<typename... TArgs> inline void convertObj(Obj& mObj, const TArgs&... mArgs) { archObj(mObj, FWD(mArgs)...); }
}
