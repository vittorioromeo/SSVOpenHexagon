...
namespace Json
{
    ...
    inline bool Reader::parse(
        std::istream& sin, Value& root, bool collectComments)
    {
        std::string doc;
        std::getline(sin, doc, (char)EOF);
        return parse(doc, root, collectComments);
    }
    inline bool Reader::parse(const char* beginDoc, const char* endDoc,
    ...
    inline bool Reader::decodeNumber(Token& token)
    {
        ...
        LargestUInt maxIntegerValue = isNegative
                                          ? LargestUInt(Value::minLargestInt)
                ...
                switch(escape)
                {
                    ...
                    case 'u':
                    {
                        ...
                        if(!decodeUnicodeCodePoint(
                               token, current, end, unicode))
                            return false;
                        decoded += codePointToUTF8(unicode);
                    }
                    break;
                    default:
                        return addError("Bad escape sequence in std::string",
                            token, current);
                }
            }
            else
    ...
    public:
        ...
        {
            SSVU_ASSERT(sizeof(AllocatedType) * objectPerAllocation >=
    ...
    inline Value::CZString::CZString(ArrayIndex mIdx)
        : cstr_(nullptr), index_(mIdx)
    {
    }
            ...
            case stringValue:
                return (value_.string_ == nullptr && other.value_.string_) ||
                       (other.value_.string_ && value_.string_ &&
                           std::strcmp(value_.string_, other.value_.string_) <
                               0);
            case arrayValue:
            case objectValue:
            {
    ...
    inline Int64 Value::asInt64() const
    {
        switch(type_)
        {
            ...
            case uintValue:
                JSON_ASSERT_MESSAGE(
                    isInt64(), "LargestUInt out of Int64 range");
    ...
    inline UInt64 Value::asUInt64() const
    {
        switch(type_)
        {
            case intValue:
                JSON_ASSERT_MESSAGE(
                    isUInt64(), "LargestInt out of UInt64 range");
    ...
    inline void StyledWriter::writeCommentAfterValueOnSameLine(
        const Value& root)
    {
