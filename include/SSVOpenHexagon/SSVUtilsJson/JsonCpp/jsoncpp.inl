...
public:
    ...
    {
        SSVOH_ASSERT(sizeof(AllocatedType) * objectPerAllocation >= sizeof(AllocatedType*));
...
private:
    ...
    static inline BatchInfo* allocateBatch(unsigned int objectsPerPage)
    {
...
inline ValueIteratorBase::ValueIteratorBase(const Value::ObjectValues::iterator& current) :
    current_(current),
    isNull_(false)
{
}
...
inline Value::Value(const char* beginValue, const char* endValue) : type_(stringValue), allocated_(true)
{
    value_.string_ = duplicateStringValue(beginValue, (unsigned int)(endValue - beginValue));
}
inline Value::Value(const std::string& value) : type_(stringValue), allocated_(true)
{
    value_.string_ = duplicateStringValue(value.c_str(), (unsigned int)value.size());
}
inline Value::Value(const StaticString& value) : type_(stringValue), allocated_(false)
{
...
inline void Value::setComment(const std::string& comment, CommentPlacement placement)
{
