...
Token& token, Location& current, Location end, unsigned int& unicode)
{
    ...
    for(int index = 0; index < 4; ++index) {
        ...
        if(c >= '0' && c <= '9')
            ...
            return addError(
            "Bad unicode escape sequence in std::string: hexadecimal digit "
            "expected.",
            token, current);
    }
    return true;
...
inline int Value::asInt() const
{
    switch(type_)
    {
        ...
        case realValue:
            JSON_ASSERT_MESSAGE(
            InRange(value_.real_, minInt, maxInt), "double out of int range");
...
inline UInt64 Value::asUInt64() const
{
    switch(type_)
    {
        ...
        case realValue:
            JSON_ASSERT_MESSAGE(
            InRange(value_.real_, 0, maxUInt64), "double out of UInt64 range");
