...
public:
    ...
    {
        items.push_back(sf::base::makeUnique<T>(
            menu, *this, mName, std::forward<TArgs>(mArgs)...));
    ...
    {
        const auto sortFunc =
            [](const sf::base::UniquePtr<ItemBase>& a,
                const sf::base::UniquePtr<ItemBase>& b)
        {
    ...
    [[nodiscard]] auto& getItem() const { return *items[index]; }
    [[nodiscard]] auto& getItems() noexcept { return items; }
    [[nodiscard]] const auto& getItems() const noexcept { return items; }
    [[nodiscard]] int getIdx() const noexcept { return index; }
