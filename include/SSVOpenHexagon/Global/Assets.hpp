...
namespace hg
{
    ...
    public:
        ...
        inline const std::unordered_map<std::string, UPtr<LevelData>>&
        getLevelDatas()
        {
            return levelDatas;
        }
        inline const LevelData& getLevelData(const std::string& mId)
        {
            return *levelDatas.at(mId);
        }
        inline const std::vector<std::string>& getLevelIdsByPack(
            const Path& mPackPath)
        {
            return levelDataIdsByPack.at(mPackPath);
        }

        inline const PackData& getPackData(const std::string& mId)
        {
        ...
        inline bool pIsValidLocalProfile()
        {
