...
struct ConfigValueBase
{
	...
	ConfigValueBase(const std::string& mLinkedName) : linkedName{mLinkedName} { }

	virtual void syncFrom(const Json::Value& mRoot) = 0;
	virtual void syncTo(Json::Value& mRoot) = 0;
};

template<typename T> struct ConfigValue : ConfigValueBase
{
	T value;

	ConfigValue(const std::string& mLinkedName) : ConfigValueBase{mLinkedName} { }

	inline operator T() { return value; }
	inline ConfigValue& operator=(T mValue) { value = mValue; return *this; }
