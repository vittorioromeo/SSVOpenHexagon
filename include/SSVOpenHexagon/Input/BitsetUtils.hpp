...
[[nodiscard]] inline FingerBitset::reference getFingerBit(
    FingerBitset& bitset, const FingerID finger) noexcept
{
    return bitset[finger];
}

[[nodiscard]] inline constexpr bool getFingerBit(
    const FingerBitset& bitset, const FingerID finger) noexcept
{
    return bitset[finger];
}

[[nodiscard]] inline KeyBitset::reference getKeyBit(
    KeyBitset& bitset, const sf::Keyboard::Key key) noexcept
{
    return bitset[toIndex(key)];
}

[[nodiscard]] inline BtnBitset::reference getBtnBit(
    BtnBitset& bitset, const sf::Mouse::Button button) noexcept
{
    return bitset[toIndex(button)];
}

[[nodiscard]] inline constexpr bool getKeyBit(
    const KeyBitset& bitset, const sf::Keyboard::Key key) noexcept
{
    return bitset[toIndex(key)];
}

[[nodiscard]] inline constexpr bool getBtnBit(
    const BtnBitset& bitset, const sf::Mouse::Button button) noexcept
{
    return bitset[toIndex(button)];
}
