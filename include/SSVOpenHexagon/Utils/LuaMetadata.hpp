...
private:
    struct FnEntry
    {
        sf::base::String fnRet;
        sf::base::String fnName;
        sf::base::String fnArgs;
        sf::base::String fnDocs;
    };

    // There are 9 categories.
    static constexpr sf::base::SizeT                           NUM_CATEGORIES = 11;
    sf::base::Array<sf::base::Vector<FnEntry>, NUM_CATEGORIES> fnEntries;
    static constexpr sf::base::Array<sf::base::StringView, NUM_CATEGORIES>
        prefixCategories{"u_", "a_", "t_", "e_", "l_", "s_", "w_", "cw_", "ct_", "shdr_", "Miscellaneous"};

    [[nodiscard]] sf::base::SizeT getCategoryIndexFromName(const sf::base::StringView fnName);

public:
    static constexpr sf::base::Array<sf::base::StringView, NUM_CATEGORIES> prefixHeaders{
        "## Utility Functions (u_)\n\n"

        "Below are the utility functions, which can be identified with the "
        "\"u_\" prefix. These are overall functions that either help "
        "utilize "
        "the game engine, be incredibly beneficial to pack developers, or "
        "help "
        ...
        "Object-oriented_programming) to allow pack developers to "
        "customize "
        "individual walls and their properties and make the most out of "
        "them.",
        ...
        "[Object-oriented "
        "programming](https://en.wikipedia.org/wiki/"
        "Object-oriented_programming) to allow pack developers to create "
        "and "
    ...
    void addFnEntry(const sf::base::String& fnRet,
                    const sf::base::String& fnName,
                    const sf::base::String& fnArgs,
                    const sf::base::String& fnDocs);

    [[nodiscard]] sf::base::SizeT getNumCategories() const noexcept;

    template <typename F>
    void forFnEntries([[maybe_unused]] F&& f, [[maybe_unused]] const sf::base::SizeT categoryIndex)
    {
