    ...
    const sf::Color& in, const float H) noexcept
{
    ...
    return sf::Color{
        //
        static_cast<sf::base::U8>((.701 * u + .168 * w) * in.r +
                                  (-.587 * u + .330 * w) * in.g +
                                  (-.114 * u - .497 * w) * in.b),

        static_cast<sf::base::U8>((-.299 * u - .328 * w) * in.r +
                                  (.413 * u + .035 * w) * in.g +
                                  (-.114 * u + .292 * w) * in.b),

        static_cast<sf::base::U8>((-.3 * u + 1.25 * w) * in.r +
                                  (-.588 * u - 1.05 * w) * in.g +
                                  (.886 * u - .203 * w) * in.b),
...
[[nodiscard, gnu::always_inline, gnu::pure]] inline constexpr sf::base::U8
