...
namespace hg
{
	...
	class MenuGame
	{
		private:
			...
			inline sf::Text& renderTextImpl(const std::string& mStr, sf::Text& mText, const Vec2f& mPosition, unsigned int mSize)
			{
			...
			inline sf::Text& renderText(const std::string& mStr, sf::Text& mText, const Vec2f& mPos, unsigned int mSize)							{ mText.setColor(getTextColor()); return renderTextImpl(mStr, mText, mPos, mSize); }
