...
public:
    bool hasUser(const std::string& mUsername) const
    {
        return users.count(mUsername) > 0;
    }
    User& getUser(const std::string& mUsername)
    {
        return users[mUsername];
    }
    void registerUser(const std::string& mUsername, const User& mUser)
    {
...
public:
    void addScore(float mDiffMult, const std::string& mUsername, float mScore)
    {
    ...
    const std::unordered_map<float, std::unordered_map<std::string, float>>&
    getScores() const
    {
        return scores;
    }
    const std::unordered_map<std::string, float>& getScores(
        float mDiffMult) const
    {
        return scores.at(mDiffMult);
    }
    const std::map<float, std::string>& getSortedScores(float mDiffMult) const
    {
        return sortedScores.at(mDiffMult);
    }
    float getPlayerScore(const std::string& mUsername, float mDiffMult) const
    {
...
public:
    bool hasLevel(const std::string& mId) const
    {
        return levels.count(mId) > 0;
    }
    LevelScoreDB& getLevel(const std::string& mId)
    {
        return levels[mId];
    }
    void addLevel(const std::string& mId, const LevelScoreDB& mDB)
    {
        levels[mId] = mDB;
    }
    const std::unordered_map<std::string, LevelScoreDB>& getLevels() const
    {
        return levels;
    }
};
...
{
    ssvuj::convertObj(mObj, "ph", mValue.passwordHash, "em", mValue.email, "st",
        mValue.stats);
}
SSVUJ_CNV_SIMPLE_END();
template <>
...
struct OHServer
{
    ...
    User& getUserFromPacket(sf::Packet& mP)
    {
    ...
    OHServer()
    {
        ...
        pHandler[FromClient::SendScore] = [this](ClientHandler& mMS,
            ...
            if(!loginDB.isLoggedIn(username))
            {
                mMS.send(buildCPacket<FromServer::SendScoreResponseInvalid>());
                return;
            }
        ...
        pHandler[FromClient::NUR_Email] = [this](ClientHandler& mMS,
                                              sf::Packet& mP) {
        ...
        pHandler[FromClient::RequestUserStats] = [this](ClientHandler& mMS,
                                                     sf::Packet& mP) {
            std::string username{
                ssvuj::getExtr<std::string>(getDecompressedPacket(mP), 0)};
            ssvuj::Obj response;
