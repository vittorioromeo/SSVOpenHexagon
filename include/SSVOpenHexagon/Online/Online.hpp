...
namespace hg
{
    ...
    namespace Online
    {
        struct GlobalThreadManager
        {
            ...
            inline void start(TFunc mFunc)
            {
                runningThreads.emplace_back(
                    std::async(std::launch::async, mFunc));
            }
        };
        ...
        public:
            void addValidator(
                const std::string& mLevelId, const std::string& mValidator)
            {
                validators[mLevelId] = mValidator;
            }
            const std::string& getValidator(const std::string& mLevelId) const
            {
                return validators.count(mLevelId) == 0
                           ? nullString
                           : validators.at(mLevelId);
            }
        };
        ...
        void tryLogin(
            const std::string& mUsername, const std::string& mPassword);

        void trySendScore(
            const std::string& mLevelId, float mDiffMult, float mScore);
        void tryRequestLeaderboard(
            const std::string& mLevelId, float mDiffMult);
        void trySendDeath();
        ...
        void tryRequestFriendsScores(
            const std::string& mLevelId, float mDiffMult);
        void trySendUserEmail(const std::string& mEmail);

        void requestLeaderboardIfNeeded(
            const std::string& mLevelId, float mDiffMult);
